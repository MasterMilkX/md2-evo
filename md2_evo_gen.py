...
class EvoMap():
    ...
    def exportMap(self, filename):
        ...
        with open(filename, "a+") as f:
            ...
            f.write(f"bv:{self.bvec2str()}")

    def bvec2str(self):
        return ''.join(str(x) for x in self.bvec)
    ...
    def getExtEval(self, filename):
        ...
        # run the script
        if self.agent == "MCTS":
            return subprocess.call(["mono", f"{self.ext_cmd}", f"{fileloc}", f"{self.agent}", f"{self.C}", f"{self.rollout}",  f"{self.playCount}"]) == 0   #make sure it works
        else:
