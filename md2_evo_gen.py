...
class EvoMap():
    def __init__(self, empty_rate, wall_rate, use_template, ext_cmd, persona, agent, C, rollout, temp_chrome_folder, randomInit=False):
    ...
    def getExtEval(self, filename):
        ...
        # run the script
        if self.agent == "MCTS":
            return subprocess.call(["mono", f"{self.ext_cmd}", f"{fileloc}", f"{self.agent}", f"{self.C}", f"{self.rollout}"]) == 0   #make sure it works
        else:
