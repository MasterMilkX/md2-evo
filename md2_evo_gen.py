...
EXT_EVAL_CMD = "python fake_md2_engine.py"     #external call to the program that evaluates the map
...
class EvoMap():
	...
	def initRandomMap(self,withWalls=True):
		...
		am = self.pairConstraint(am,'portal')
	...
	def mutateMap(self,mutRate):
		...
		#make sure there are an even number of portals
		nm = self.pairConstraint(nm,'portal')
...
class FI2Pop():
	def __init__(self,mapMutRate,maxSets,feasSelRate):
		...
		self.feasRate = feasSelRate #probability to select from the feasible set for the new population
	...
	def exportArc(self,folderLoc,expInfeas=False):
		os.makedirs(folderLoc, exist_ok=True)   #make the folder if it doesn't exist yet
	...
	def sortPop(self):
		for p in self.population:
			gs = "".join([str(pi) for pi in p.genome])   #genome as a string binary value
			c = p.con
	...
	def evolvePop(self,popsize,iterations,parallel=False,outputArc=True):
		...
		with tqdm(total = iterations) as pbar:
			for i in range(iterations):
				...
				pbar.update(1)
				pbar.set_description(f"F: {[o.fitness for o in self.feas]} | Is: {[e.canTraverse() for e in self.infeas]}")
